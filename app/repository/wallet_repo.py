...
class ReferralEarningRepository(BaseRepository[ReferralEarning, dict, dict]):
    ...
    async def count_by_referred_user(self, referred_user_id: UUID) -> int:
        """Count total earnings generated by a referred user"""
        query = select(ReferralEarning).where(ReferralEarning.referred_user_id == referred_user_id)
        result = await self.session.exec(query)
        return len(result.all())
